...
const groupColumnsByPrefix = (columns: string[]): Record<string, string[]> => {
  ...
  columns.forEach((col) => {
    ...
    else if (col === "PUREID" || col === "PUREGOLDNAME") groups["PURE"].push(col);
...
function StockReport() {
  ...
  const { headers, subHeaders, tableData, columnWidthMap } = useMemo(() => {
    ...
    ["PUREID", "PURENAME"].forEach((c) => {
      if (allColumns.includes(c)) columnsToShow.push(c);
    });
    ["PUREID", "PUREGOLDNAME", "METALID", "METALNAME",  "SRC"].forEach(
      (c) => {
        if (allColumns.includes(c)) columnsToShow.push(c);
      }
    );
    ...
    const formattedData = reportData.map((pure: any, index: number) => ({
      ...pure,
      _id: pure.PUREID ? `${pure.PUREID}_${index}` : `row_${index}`,
    }));
