...
const groupColumnsByPrefix = (columns: string[]) => {
  const groups: Record<string, string[]> = {
    'ITEM': [], // Special group for item info
    'OP': [],   // Opening
    'IS': [],   // Issue
    'RE': [],   // Receive
    'CL': []    // Closing
  ...
  columns.forEach(col => {
    if (col.startsWith('OP_')) {
      ...
      groups['CL'].push(col);
    } else if (col === 'ITEMID' || col === 'ITEMNAME') {
      groups['ITEM'].push(col);
    }
  });
...
function StockReport() {
  ...
  const { data: reportData, isLoading, isError, refetch } = useItemStockReport();
  ...
  const { headers, subHeaders, tableData } = useMemo(() => {
    ...
    if (groupedColumns['ITEM'].length > 0) {
      generatedHeaders.push({
        key: 'ITEM',
        label: groupLabels['ITEM'],
        align: 'center' as const,
        bg: groupColors['ITEM'].bg,
        color: groupColors['ITEM'].color,
      });

      // Add sub-headers for ITEM group
      groupedColumns['ITEM'].forEach(col => {
        generatedSubHeaders.push({
          key: col,
          label: col === 'ITEMID' ? 'Item ID' : 'Item Name',
          headerKey: 'ITEM',
          align: 'start',
          bg: groupColors['ITEM'].subBg,
          color: groupColors['ITEM'].subColor
        });
      });
    }
    ...
    const formattedData = reportData.map((item: any, index: number) => ({
      ...item,
      _id: item.ITEMID || index // Ensure unique ID for each row
    }));
    ...
    return (
      <>
        {orderedColumns.map(columnKey => {
          ...
          const isItemColumn = columnKey === 'ITEMID' || columnKey === 'ITEMNAME';
          const isNumeric = !isItemColumn && typeof value === 'number';
        ...
        {showReport && tableData.length > 0 && (
          <>
                ...
                totals={{
                  ...
                  calculateTotals: (data) => {
                    ...
                    subHeaders.forEach(sh => {
                      if (sh.key !== 'ITEMID' && sh.key !== 'ITEMNAME') {
            ...
            <HStack gap={4} p={2} bg="gray.50" borderRadius="md">
              <Box>
                <Text fontSize="xs" color="gray.600">Total Items</Text>
                ...
                <>
                  <Box>
                    ...
                    <Text fontSize="md" fontWeight="bold">
                      {tableData.reduce((sum, item) => sum + (item.OP_GRSWT || 0), 0).toFixed(3)}
                    </Text>
                  </Box>
                  <Box>
                    <Text fontSize="xs" color="gray.600">Total Closing Gross Wt</Text>
                    <Text fontSize="md" fontWeight="bold">
                      {tableData.reduce((sum, item) => sum + (item.CL_GRSWT || 0), 0).toFixed(3)}
                    </Text>
                  </Box>
                </>
              )}
        ...
        {!showReport && (
          ...
          >
            <Text fontSize="lg" color="gray.600" mb={4}>
              Click "Get Report" to view the item-wise stock report
            </Text>
            <Text fontSize="sm" color="gray.500">
              The report will show opening, issued, received, and closing balances for all items
            </Text>
          </Box>
        )}
