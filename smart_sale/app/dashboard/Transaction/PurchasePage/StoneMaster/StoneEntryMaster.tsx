...
export default function StoneEnterMaster({
    ...
    const errors = useMemo<Record<string, string>>(() => {
        ...
        rows.forEach((row, ri) => {
            if(row.stoneId){
                ...
                if (!row.stoneUnit || row.stoneUnit.trim() === "")
                    errs[`${ri}_stoneUnit`] = "Unit is required";
                if (!row.stoneCalculation || row.stoneCalculation.trim() === "")
                    errs[`${ri}_stoneCalculation`] = "Calculation is required";
                const weight = Number(row.stoneWeight);
    ...
    const handleSaveAndClose = useCallback(() => {
        ...
        if (totalUsedWeight > Number(grsWeight)) {
            toaster.create({
        ...
        onSave(saveRows);
        onClose();
    }, [rows, errors, totalUsedWeight, grsWeight, onSave, onClose]);
