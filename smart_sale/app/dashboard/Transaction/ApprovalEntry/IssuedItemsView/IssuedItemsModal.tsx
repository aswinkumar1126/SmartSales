...
export default function SalesBillViewModal({
    ...
    const renderRowCells = (row: any) => {
        return (
            <>
                <Table.Cell>{renderCellValue(row.ENTRYNO)}</Table.Cell>
                ...
                <Dialog.Positioner>
                    <Dialog.Content minW={'80vw'} bg={theme.colors.primary}>
                        ...
                        <Dialog.Body p={4} >
                            <Box mb={4} display="flex" gap={3}>
                                ...
                                <Box display={'flex'} alignItems={'center'}>
                                    ...
                                    <DatePickerInput
                                        value={billParams.TRANDATE}
                                        onChange={(date) =>
                                            onBillParamChange("TRANDATE", date)
                                        }
                        ...
                        <Dialog.Footer m={0}>
                            ...
                            onClick={()=>
                                {
                                    loadApprovalReceiptItems(selectedItems)
                                    setSelectedItems([]);
                                    setSelectedItemIds([]);
                                    onClose();
                                }
