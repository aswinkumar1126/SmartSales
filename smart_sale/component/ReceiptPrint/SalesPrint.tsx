...
const buildThermalHTML = (p: SalesReceiptConfig, softData?: SoftControl, is50?: boolean): string => {
  ...
  const itemSections = sections.map(({ label, rows, type }) => {
    ...
    /**
     * =====================================================
     * CHECK DYNAMIC STONE COLUMNS (from STONEDETAILS)
     * =====================================================
     */
    const hasStoneWt = rows.some((item) =>
      (item?.STONEDETAILS || []).some(
        (s: any) => Number(s.stoneId) === 9999
      )
    );

    const hasNavaWt = rows.some((item) =>
      (item?.STONEDETAILS || []).some(
        (s: any) => Number(s.stoneId) === 9997
      )
    );

    const hasDiamondWt = rows.some((item) =>
      (item?.STONEDETAILS || []).some(
        (s: any) => Number(s.stoneId) === 9998
      )
    );
    ...
    if (rows.length > 0) {
      if (isSalesType) {
        ...
        // Calculate stone weights and amounts from stoneDetails
        rows.forEach((item) => {
    ...
    /**
     * =====================================================
     * HEADERS with dynamic stone columns
     * =====================================================
     */
...
<tr>`;
    ...
    /**
     * =====================================================
     * ROWS with stone weight calculations
     * =====================================================
     */
    const rowsHtml = rows
      .map((item) => {

        console.log(item,'itempuregoldname')
        const name =
          item.ITEMNAME ||
          item.PUREGOLDNAME ||
...
<tr style="font-size:10px">
  ...
  <td style="border:1px solid #000; padding:2px; text-align:right">${fmtWt(item.PUREWT)}</td>
    ...
    /**
     * =====================================================
     * TOTAL ROW with dynamic stone columns
     * =====================================================
     */
...
<tr style="font-size:10px">
  ...
  <td style="border:1px solid #000; padding:2px; text-align:right;"><strong>${fmtWt(totals.purewt)}</strong></td>
     ...
     * =====================================================
     */
    const extraChargesTable = totals && isSalesType && (totals.hmc || totals.mc || totals.stoneAmt || totals.navaAmt || totals.diamondAmt)
      ? `
...
<div class="pr-thermal">
  ...
  <div style="text-align:center; font-size:12px; font-weight:bold; letter-spacing:0.5px; margin:4px 0;">SALE ESTIMATION</div>
...
.pr-thermal table.no-border { border-collapse:collapse; margin:2px 0; }
.pr-thermal table.with-border { border-collapse:collapse; margin:4px 0; border:1px solid #222; }
...
.pr-thermal .tran-table { margin: 10px 0px; }
`;
};
