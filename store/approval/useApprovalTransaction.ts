...
interface ApprovalTransactionState {
    ...
    selectedTransactionTypes: ApprovalTransactionType[];
    draftRows: DraftRow[];
    editingState: {
        rowId: string | null;
    ...
    // Actions
    setSelectedTransactionTypes: (types: ApprovalTransactionType[]) => void;
    addTransactionType: (type: ApprovalTransactionType) => void;
...
export const useApprovalTransactionStore = create<ApprovalTransactionState>()(
    persist(
        (set, get) => ({
            // Initial state
        ...
        {
            name: "approval-transaction-storage",
